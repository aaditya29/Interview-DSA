...
public:
    ...
    pair<vector<int>, vector<int>> solve(int node, vector<int> &present, vector<int> &future, int b)
    {
        ...
        for (int i = full; i <= b; i++)
            res0[i] = max(res0[i], dp1[i - full] + profitFull);
        for (int i = half; i <= b; i++)
            res1[i] = max(res1[i], dp1[i - half] + profitHalf);
...
int main()
{
    Solution sol;
    int n = 5;
    vector<int> present = {100, 200, 300, 400, 500};
    vector<int> future = {150, 250, 350, 450, 600};
    vector<vector<int>> hierarchy = {{1, 2}, {1, 3}, {2, 4}, {2, 5}};
    int budget = 700;
    int result = sol.maxProfit(n, present, future, hierarchy, budget);
